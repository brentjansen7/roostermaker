...
export default function SchoolroosterLijst() {
  ...
  async function verwijderen(e, id) {
    e.preventDefault();
    e.stopPropagation();
    if (!confirm('Schoolrooster verwijderen? Dit verwijdert ook alle ingeplande lessen en conflicten.')) return;
    try {
      await schoolroostersApi.verwijderen(id);
      setRoosters(prev => prev.filter(r => r.id !== id));
      toonToast('Schoolrooster verwijderd', 'succes');
    } catch (err) {
